process GRANDQC_ARTIFACT {
    ...
    """
    echo "Generating Artifact Segmentation"

    python $OME_TIFF_QC/main.py \\
    --slide_folder "${slide_folder}" \\
    --output_dir "${output_dir}" \\
    --create_geojson "${create_geojson}" \\
    --mpp_model "${qc_mpp_model}"

    if [ $? -eq 0 ]; then echo Artifact Segmentation compeleted!; else echo Artifact Segmentation failed; fi
